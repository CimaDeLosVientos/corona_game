...
from .helpers import *
from .things import Video, Laptop, LowBattery
...
class Level3Play(Scene):
    def __init__(self):
        ...
        self.object_2_icon = Laptop(OBJECT_2_ICON_LOCATION)
    ...
    def on_update(self, time):
        if not self.start:
            ...
            return
        elif (self.player.score["video"] >= OBJECT_1_NEEDS_LEVEL_1
            and self.player.score["laptop"] >= OBJECT_2_NEEDS_LEVEL_1):
        ...
        if lottery < RATIO_OBJECT_1_LEVEL_1:
            self.things.Video(laptop(((random.randrange(LEFT_LIMIT, RIGHT_LIMIT), -50))))
        elif lottery < RATIO_OBJECT_2_LEVEL_1:
            self.things.add(Laptop(((random.randrange(LEFT_LIMIT, RIGHT_LIMIT), -50))))
    ...
    def on_draw(self, screen):
        ...
        object_1_score, object_1_score_rect = draw_text(str(self.player.score["video"]), OBJECT_1_COUNTER_LOCATION[0], OBJECT_1_COUNTER_LOCATION[1])
        screen.blit(object_1_score, object_1_score_rect)

        screen.blit(self.object_2_icon.image, self.object_2_icon.rect)
        object_2_score, object_2_score_rect = draw_text(str(self.player.score["laptop"]), OBJECT_2_COUNTER_LOCATION[0], OBJECT_2_COUNTER_LOCATION[1])
